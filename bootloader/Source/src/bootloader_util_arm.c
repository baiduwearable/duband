...
{
    #ifdef DEBUG_LOG
        char out_put_buffer[30];
        simple_uart_putstring((const uint8_t *)"the value of MSP is : ");
        sprintf(out_put_buffer,"%x",*((uint32_t *)start_addr)); //start of stack address
        simple_uart_putstring((const uint8_t *)out_put_buffer);
        simple_uart_putstring((const uint8_t *)"\r\n");
    
        simple_uart_putstring((const uint8_t *)"reset handle address of application is  : ");
        sprintf(out_put_buffer,"%x",*((uint32_t *)(start_addr + 4)));
        simple_uart_putstring((const uint8_t *)out_put_buffer);
        simple_uart_putstring((const uint8_t *)"\r\n");
    #endif 
    //error handle
    if((*((uint32_t *)start_addr) < 0x20002000) || (*((uint32_t *)start_addr) > 0x20004000) || (*((uint32_t *)(start_addr + 4)) < 0x14000) || (*((uint32_t *)(start_addr + 4)) > 0x23000)) {
        return;
    }
