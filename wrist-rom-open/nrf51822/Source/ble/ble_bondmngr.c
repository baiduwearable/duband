...
uint32_t ble_bondmngr_bonded_master_delete(uint16_t master_id)
{
   ...
   /******************************************************************
    * update m_master information after delete one bond info
    *******************************************************************/
    if((m_master.bond.master_handle > master_handle_to_be_deleted) && (m_master.bond.master_handle >= 1)) {
       m_master = m_masters_db[m_master.bond.master_handle - 1];
    }
