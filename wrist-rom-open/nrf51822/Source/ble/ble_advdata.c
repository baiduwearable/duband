...
/********************************************************************
* Random device address encode
*********************************************************************/
#define RANDOM_DEVICE_ADRESS_LEN (6)
uint32_t random_device_address_encode(uint8_t *             p_encoded_data,
                                             uint8_t *             p_len)
{
    //random device address length = 6
    if(*p_len + 2 + RANDOM_DEVICE_ADRESS_LEN > BLE_GAP_ADV_MAX_SIZE) {
         return NRF_ERROR_DATA_SIZE;
    }
    ...
    **********************************************/
    ...
    p_encoded_data[(*p_len)++] = 2+ RANDOM_DEVICE_ADRESS_LEN;
    p_encoded_data[(*p_len)++] = BLE_GAP_AD_TYPE_RANDOM_TARGET_ADDRESS;
    
    int8_t i = 0;
    // show as big endian
    for(i = RANDOM_DEVICE_ADRESS_LEN - 1 ;i>=0;--i) {
       p_encoded_data[(*p_len)++] =  (mac_address >> 8*i) & 0xff;
    }
...
/************************************************************************
* encode device Class information  
*************************************************************************/
#define BLUETOOTH_DEV_CLASS (0x704)
uint32_t encode_device_class(uint8_t * p_encoded_data,uint8_t * p_len) 
{
    //BLE_GAP_AD_TYPE_CLASS_OF_DEVICE
    if (*p_len + 2 + sizeof(uint16_t) > BLE_GAP_ADV_MAX_SIZE) {
        return NRF_ERROR_DATA_SIZE;
    }

    p_encoded_data[(*p_len)++] = 2 + sizeof(uint16_t);
    p_encoded_data[(*p_len)++] = BLE_GAP_AD_TYPE_CLASS_OF_DEVICE;

    //encode type info into array
    (*p_len) += uint16_encode(BLUETOOTH_DEV_CLASS, &p_encoded_data[*p_len]);
